import re
a = """eyr:2024 pid:662406624 hcl:#cfa07d byr:1947 iyr:2015 ecl:amb hgt:150cm
...
def task7():
    j = 0
    for y in c:
        if "byr" in y and "iyr" in y and "eyr" in y and "hgt" in y and "hcl" in y and "ecl" in y and "pid" in y:
            k.append(y)
            j = j + 1
    return j
...
def task8():
    ...
    for y in l:
        ...
        if 1920 <= a1 <= 2002:
            if 2010 <= a6 <= 2020:
                if 2020 <= a3 <= 2030:
                    if (a5.endswith("cm") and (150 <= int(a5[:-2]) <= 193)) or (a5.endswith("in") and (59 <= int(a5[:-2]) <= 76)):
                        if len(a4) == 7 and a4.startswith("#") and re.search("[0-9a-f]", a4):
                            if len(a2) == 3 and ("amb" in a2 or "blu" in a2 or "brn" in a2 or "gry" in a2 or "grn" in a2 or "hzl" in a2 or "oth" in a2):
                                if len(a7) == 9 and a7.isnumeric():
                                    q += 1
    return q
